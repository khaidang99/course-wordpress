				...
				if( !empty( $font_uniq ) ) {
					...
					foreach ( $font_uniq as $value ) {
						...
						if( !empty( $value ) ) {
							...
							echo "<link href='$ssl_control://fonts.googleapis.com/css?family=" . $font_name . ":200,300,400,500,600,700&subset=" . $ot_font_subset_echo . "' rel='stylesheet' type='text/css'>\n";
	...
	*
	...
	if( !function_exists( 'eventchamp_options_name' ) ) {
		...
		function eventchamp_options_name() {
			...
			$html = esc_html__( 'Created by GloriaThemes', 'eventchamp' );
