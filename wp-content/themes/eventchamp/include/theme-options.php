...
*
======*/
if( !class_exists( 'OT_Loader' ) or !class_exists( 'eventchamp_core_plugin' ) ) {

	if( !function_exists( 'eventchamp_default_fonts_url' ) ) {

		function eventchamp_default_fonts_url() {

			$custom_fonts_list = array();

			/*====== Customize Variables ======*/
			$latin_extended = get_theme_mod( 'eventchamp_typography_latin_extended', '1' );
			$cyrillic_extended = get_theme_mod( 'eventchamp_typography_cyrillic_extended', '1' );
			$greek_extended = get_theme_mod( 'eventchamp_typography_greek_extended', '1' );
			$font_weights = get_theme_mod( 'eventchamp_font_weights', '100,100italic,200,200italic,300,300italic,400,400italic,500,500italic,600,600italic,700,700italic,800,800italic,900,900italic' );
			...
			/*====== Fonts ======*/
			$custom_fonts_list[] = "Poppins";

			/*====== Fonts Filter ======*/
			if( !empty( $custom_fonts_list ) ) {

				$custom_fonts_list = array_unique( $custom_fonts_list );

				$prefixed_array = preg_filter( '/$/', $weights, $custom_fonts_list );
	...
	if( !function_exists( 'eventchamp_default_fonts' ) ) {

		function eventchamp_default_fonts() {

			wp_enqueue_style( 'eventchamp-fonts', eventchamp_default_fonts_url(), array(), '1.0.0' );

		}
		add_action( 'wp_enqueue_scripts', 'eventchamp_default_fonts' );
		add_action( 'admin_enqueue_scripts', 'eventchamp_default_fonts' );

	}

	if( !function_exists( 'eventchamp_default_fonts_customizer' ) ) {

		function eventchamp_default_fonts_customizer() {

			$custom = "";
			$custom .= 'body,.ui-widget-content, input[type="email"], input[type="number"], input[type="password"], input[type="tel"], input[type="url"], input[type="text"], input[type="time"], input[type="week"], input[type="search"], input[type="month"], input[type="datetime"], input[type="date"], textarea, textarea.form-control, select, .woocommerce form .form-row .select2-container .select2-choice, .upload-input, .select2-dropdown, .select2-search--dropdown .select2-search__field, .select2-container--default .select2-search--dropdown .select2-search__field, .select2-container--default .select2-selection--single, #add_payment_method table.cart td.actions .coupon .input-text, .woocommerce-cart table.cart td.actions .coupon .input-text, .woocommerce-checkout table.cart td.actions .coupon .input-text, .form-control, .bootstrap-select.gt-select > button, .woocommerce #respond input#submit.alt, .woocommerce a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt, .woocommerce #respond input#submit, .woocommerce a.button, .woocommerce button.button, .woocommerce input.button, button, input[type="submit"], .gt-header .gt-navbar .gt-menu{font-family: "Poppins";}';

			wp_enqueue_style( 'eventchamp-custom', get_template_directory_uri() . '/include/assets/css/custom.css', array(), '1.0.0' );
			wp_add_inline_style( 'eventchamp-custom', $custom );

		}
		add_action( 'wp_enqueue_scripts', 'eventchamp_default_fonts_customizer' );

	}

	if( !function_exists( 'eventchamp_default_admin_fonts_customizer' ) ) {

		function eventchamp_default_admin_fonts_customizer() {

			$custom = "";
			$custom .= ".editor-block-list__layout{font-weight: 400; font-size: 16px; line-height: 1.15; color: #111111;}";
			$custom .= ".editor-block-list__layout > *, .editor-styles-wrapper p, .editor-styles-wrapper p.wp-block-paragraph{font-family: 'Frank Ruhl Libre'; font-size: 1rem; line-height: 1.7;}";
			$custom .= ".editor-post-title .editor-post-title__input, .editor-block-list__layout h1, .editor-block-list__layout h2, .editor-block-list__layout h3, .editor-block-list__layout h4, .editor-block-list__layout h5, .editor-block-list__layout h6, .wp-block-latest-comments, .wp-block-latest-posts li, label, input, button,{font-family: 'Poppins';}";

			wp_enqueue_style( 'eventchamp-custom', get_template_directory_uri() . '/include/assets/css/custom.css', array(), '1.0.0' );
			wp_add_inline_style( 'eventchamp-custom', $custom );

		}
		add_action( 'admin_enqueue_scripts', 'eventchamp_default_admin_fonts_customizer' );
